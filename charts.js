...
function buildCharts(sample) {
  ...
  d3.json("static/js/samples.json").then((data) => {
    ...
    // 3.1  Create a variable that holdes the metadata array & filters metadata for the object with the desired sample number.
      var metadata = data.metadata
    ...
    // 3.3 Create variable that holds the washing frequency
    ...
    // Hint: Get the the top 10 otu_ids and map them in descending order so the otu_ids with the most bacteria are last. 
      ...
      var xValue = sample_values.slice(0,10).reverse()
